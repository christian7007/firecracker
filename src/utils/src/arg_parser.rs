...
impl<'a> Argument<'a> {
    ...
    /// If `allow_duplicates` is true, then the user can provide multiple values for the
    /// argument. It sets the `takes_value` option to true, so the user must provides at least
    /// one value.
...
impl<'a> Arguments<'a> {
    ...
    fn validate_arg(&self, arg: &str) -> Result<()> {
        ...
        // provided more than once (unless allow_duplicates is set).
        let argument = self
            .args
            .get(arg_name)
            .ok_or_else(|| Error::UnexpectedArgument(arg_name.to_string()))?;

        if !argument.allow_duplicates && argument.user_value.is_some() {
...
mod tests {
    ...
    fn test_duplicates() {
        let arg_parser = ArgParser::new()
            .arg(
                Argument::new("no-duplicates")
                    .takes_value(true)
                    .help("argument that takes just one value."),
            )
            .arg(
                Argument::new("duplicates")
        ...
        let args = vec!["binary-name", "--no-duplicates", "1", "--no-duplicates", "2"]
            .into_iter()
            .map(String::from)
            .collect::<Vec<String>>();
        ...
        assert_eq!(
            arguments.parse(&args),
            Err(Error::DuplicateArgument("no-duplicates".to_string()))
        );

        arguments = arg_parser.arguments().clone();

        // Check single value arguments works as expected when jus one value
        // is provided for both arguments.
        let args = vec!["binary-name", "--no-duplicates", "1", "--duplicates", "2"]
            .into_iter()
            .map(String::from)
            .collect::<Vec<String>>();
        ...
        // Check duplicates arg allow multiple values
        let args = vec!["binary-name", "--duplicates", "1", "--duplicates", "2"]
            .into_iter()
            .map(String::from)
            .collect::<Vec<String>>();
